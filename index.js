...
function getTC(guildId) {
  if (!ticketConfigs.has(guildId)) ticketConfigs.set(guildId, {
    categoryId: null, supportRoleId: null, logChannelId: null,
    panelChannelId: null,
    panelMessage: "Click the button below to open a support ticket.",
    openTickets: new Map(),
  });
...
async function sendTicketPanel(channel, tc) {
  ...
  await channel.send({ embeds: [new EmbedBuilder().setColor(0x5865f2).setTitle("🎫 Support Tickets").setDescription(tc.panelMessage)], components: [row] });
}
async function handleTicket(message, args) {
  ...
  if (sub === "setup") {
    if (!requirePerm(message, PermissionFlagsBits.Administrator)) return;
    const targetCh = tc.panelChannelId ? message.guild.channels.cache.get(tc.panelChannelId) : message.channel;
    if (!targetCh) { await message.reply("❌ Panel channel not found. Set it first: `+ticket channel #ch`"); return; }
    await sendTicketPanel(targetCh, tc);
    await message.reply(`✅ Ticket panel created in <#${targetCh.id}>!`);
  } else if (sub === "channel") {
    if (!requirePerm(message, PermissionFlagsBits.Administrator)) return;
    const ch = message.mentions.channels.first(); if (!ch) { await message.reply("Mention a channel! Usage: `+ticket channel #channel`"); return; }
    tc.panelChannelId = ch.id; await message.reply(`✅ Ticket panel channel set to <#${ch.id}>. Now run \`+ticket setup\` to send the panel.`);
  } else if (sub === "message") {
    if (!requirePerm(message, PermissionFlagsBits.Administrator)) return;
    const msg = args.slice(1).join(" "); if (!msg) { await message.reply("Provide a message! Usage: `+ticket message <your message>`"); return; }
    tc.panelMessage = msg; await message.reply({ embeds: [new EmbedBuilder().setColor(0x5865f2).setTitle("✅ Ticket Panel Message Updated").setDescription(`**New message:**\n${msg}\n\nRun \`+ticket setup\` to resend the panel with the new message.`)] });
  } else if (sub === "close") {
    ...
    await message.reply({ embeds: [new EmbedBuilder().setColor(0x5865f2).setTitle("🎫 Ticket Commands").addFields(
      { name: "Setup", value: "`+ticket channel #ch` — Set panel channel\n`+ticket message <text>` — Set panel embed message\n`+ticket setup` — Send the panel", inline: false },
      { name: "Config", value: "`+ticket category #cat` — Set ticket category\n`+ticket supportrole @role` — Set support role\n`+ticket logchannel #ch` — Set log channel", inline: false },
      { name: "Manage", value: "`+ticket close` — Close current ticket\n`+ticket add @user` — Add user to ticket\n`+ticket remove @user` — Remove user from ticket", inline: false },
    )] });
  }
}
...
async function handleHelp(message) {
  await message.reply({ embeds: [new EmbedBuilder().setColor(0x5865f2).setTitle("📖 Bot Commands").setDescription("All commands (prefix: `+`)")
    .addFields(
      ...
      { name: "🎫 Tickets", value: "`+ticket channel #ch` `+ticket message <text>` `+ticket setup` `+ticket close` `+ticket add/remove @u` `+ticket category/supportrole/logchannel`" },
