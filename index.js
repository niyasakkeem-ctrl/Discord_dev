const { Client, GatewayIntentBits, PermissionsBitField, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, AttachmentBuilder } = require('discord.js');
...
async function createWelcomeCard(member) {
  ...
  ctx.lineWidth = 4;
  ctx.strokeRect(4, 4, 792, 292);

  // Inner subtle border
  ctx.strokeStyle = 'rgba(191, 0, 255, 0.3)';
  ctx.lineWidth = 1;
  ctx.strokeRect(12, 12, 776, 276);

  // Avatar glow
  const avatarX = 150, avatarY = 150, avatarR = 80;
  const glowGrad = ctx.createRadialGradient(avatarX, avatarY, 40, avatarX, avatarY, 100);
  ...
  ctx.shadowColor = '#bf00ff';
  ctx.shadowBlur = 8;
  const serverName = member.guild.name.length > 22 ? member.guild.name.slice(0, 22) + '...' : member.guild.name;
  ctx.fillText(`📡 ${serverName}`, 270, 180);

  // Member count
  ctx.shadowBlur = 5;
  ctx.fillStyle = '#aaaaaa';
  ctx.font = '18px sans-serif';
  ctx.fillText(`👾 Member #${member.guild.memberCount}`, 270, 220);

  // Footer
  ctx.font = '13px sans-serif';
  ctx.fillStyle = 'rgba(0,255,255,0.4)';
  ctx.shadowBlur = 0;
  ctx.fillText('Made by Niyas 💙', 270, 268);
...
client.on('messageCreate', async (message) => {
  ...
  if (command === 'help') {
    return message.reply({ embeds: [new EmbedBuilder().setColor('Blurple').setTitle('📖 Bot Commands').setThumbnail(client.user.displayAvatarURL()).addFields(
      ...
      { name: '👋 Welcome', value: 'Auto welcome card when new member joins!', inline: false }
  ...
  // ── ABOUT ──
  if (command === 'about') {
    return message.reply({ embeds: [new EmbedBuilder().setColor('Blurple').setTitle('🤖 About This Bot').setThumbnail(client.user.displayAvatarURL()).setDescription('A powerful all-in-one Discord bot with moderation, fun, and more!').addFields({ name: '👨‍💻 Made by', value: 'Niyas', inline: true }, { name: '⚙️ Built With', value: 'Node.js + discord.js', inline: true }, { name: '🛡️ Features', value: 'Anti-Spam, Anti-Nuke, Anti-Link, Tickets, Moderation, Fun, Welcome Card', inline: false }, { name: '📌 Prefix', value: `\`${PREFIX}\``, inline: true }, { name: '🏓 Ping', value: `${client.ws.ping}ms`, inline: true }).setFooter({ text: 'Made by Niyas 💙' }).setTimestamp()] });
  }
