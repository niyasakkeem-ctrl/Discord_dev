...
// ─── TICKETS (YAMALOGAM - 2 CATEGORY SYSTEM) ────────────────────────────────
const ticketConfigs = new Map();
function getTC(guildId) {
  if (!ticketConfigs.has(guildId)) ticketConfigs.set(guildId, {
    gameSupportCategoryId: null,  // 🎮 Game Support category
    buyProductCategoryId: null,   // 🛒 Buy Product category
    supportRoleId: null,
    logChannelId: null,
    openTickets: new Map(), // userId → { channelId, type }
  });
...
const TICKET_TYPES = {
  game_support: {
    emoji: "🎮",
    label: "விளையாட்டு உதவி",
    labelEn: "Game Support",
    color: 0x5865f2,
    channelPrefix: "game-ticket",
    description: "விளையாட்டு சம்பந்தமான எந்த உதவியும் இங்கே கேளுங்கள்!\nGame related issues, bugs, questions — ask here!",
    footerText: "🎮 YAMALOGAM | விளையாட்டு உதவி",
  },
  buy_product: {
    emoji: "🛒",
    label: "பொருட்கள் வாங்க",
    labelEn: "Buy Product",
    color: 0xf1c40f,
    channelPrefix: "buy-ticket",
    description: "எந்த பொருளும் வாங்க விரும்புகிறீர்களா? இங்கே கேளுங்கள்!\nWant to purchase something? Create a ticket here!",
    footerText: "🛒 YAMALOGAM | பொருட்கள் வாங்க",
  },
};

// Send the main ticket panel with select menu
async function sendTicketPanel(channel) {
  const selectMenu = new StringSelectMenuBuilder()
    .setCustomId("ticket_select_type")
    .setPlaceholder("📋 உங்கள் ticket வகையை தேர்ந்தெடுக்கவும் | Select your ticket type")
    .addOptions(
      new StringSelectMenuOptionBuilder()
        .setLabel("🎮 விளையாட்டு உதவி | Game Support")
        .setDescription("Game bugs, issues, help — விளையாட்டு பிரச்சனைகள்")
        .setValue("game_support")
        .setEmoji("🎮"),
      new StringSelectMenuOptionBuilder()
        .setLabel("🛒 பொருட்கள் வாங்க | Buy Product")
        .setDescription("Purchase items, shop queries — பொருட்கள் வாங்க")
  ...
  const embed = new EmbedBuilder()
    .setColor(0x2b2d31)
    .setTitle("🎫 ꜱᴜᴘᴘᴏʀᴛ ᴛɪᴄᴋᴇᴛ | YAMALOGAM")
    .setDescription(
      "```\n╔══════════════════════════════╗\n║   🌟 வணக்கம் / WELCOME 🌟    ║\n╚══════════════════════════════╝\n```\n" +
      "உதவி தேவையா? கீழே உங்கள் வகையை தேர்ந்தெடுங்கள்!\n" +
      "Need help? Select your category below!\n\n" +
      "🎮 **விளையாட்டு உதவி** — Game issues, bugs, help\n" +
      "🛒 **பொருட்கள் வாங்க** — Buy products, shop queries\n\n" +
      "⚠️ *ஒரே நேரத்தில் ஒரு ticket மட்டுமே திறக்க முடியும்!*\n" +
      "*Only 1 ticket allowed at a time!*"
    )
    .setThumbnail("https://i.imgur.com/AfFp7pu.png")
    .setFooter({ text: "⚡ YAMALOGAM Bot | யாமாலோகம்" })
    .setTimestamp();
...
async function handleTicketSelectType(interaction) {
  if (!interaction.guild || !interaction.isStringSelectMenu()) return;
  const type = interaction.values[0];
  const tc = getTC(interaction.guild.id);
  const member = interaction.member;
  const cfg = TICKET_TYPES[type];
  if (!cfg) return;

  // Check existing ticket
  if (tc.openTickets.has(member.id)) {
    const existing = tc.openTickets.get(member.id);
    await interaction.reply({ content: `⚠️ உங்களுக்கு ஏற்கனவே ஒரு ticket உள்ளது! | You already have a ticket: <#${existing.channelId}>`, ephemeral: true });
    return;
  }
  ...
  try {
    ...
    const parentId = type === "game_support" ? tc.gameSupportCategoryId : tc.buyProductCategoryId;
    ...
    if (tc.supportRoleId) overwrites.push({
      id: tc.supportRoleId, type: OverwriteType.Role,
      allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory, PermissionFlagsBits.ManageMessages],
    });

    const ch = await interaction.guild.channels.create({
      name: `${cfg.channelPrefix}-${member.user.username}`,
      type: ChannelType.GuildText,
      parent: parentId ?? undefined,
      permissionOverwrites: overwrites,
    });

    tc.openTickets.set(member.id, { channelId: ch.id, type });

    const closeRow = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId("ticket_close").setLabel("🔒 Ticket மூட | Close Ticket").setStyle(ButtonStyle.Danger)
    );

    const ticketEmbed = new EmbedBuilder()
      .setColor(cfg.color)
      .setTitle(`${cfg.emoji} ${cfg.labelEn} Ticket | ${cfg.label}`)
      .setDescription(
        `வணக்கம் <@${member.id}>! 👋\n\n` +
        `${cfg.description}\n\n` +
        "```\n⏳ எங்கள் team விரைவில் உங்களுக்கு பதில் அளிப்பார்கள்!\n   Our team will respond to you shortly!\n```\n" +
        `> 🔒 Ticket மூட: **ticket_close** button அழுத்துங்கள்`
      )
      .addFields(
        { name: "👤 User", value: `<@${member.id}>`, inline: true },
        { name: "📂 Category", value: `${cfg.emoji} ${cfg.label}`, inline: true },
        { name: "📅 Opened", value: `<t:${Math.floor(Date.now() / 1000)}:R>`, inline: true },
      )
      .setFooter({ text: cfg.footerText })
      .setTimestamp();

    await ch.send({
      content: `<@${member.id}>${tc.supportRoleId ? ` <@&${tc.supportRoleId}>` : ""}`,
      embeds: [ticketEmbed],
      components: [closeRow],
    });

    await interaction.editReply({ content: `✅ உங்கள் ticket உருவாக்கப்பட்டது! | Ticket created: <#${ch.id}>` });

    if (tc.logChannelId) {
      const logEmbed = new EmbedBuilder()
        .setColor(cfg.color)
        .setTitle("📋 புதிய Ticket திறக்கப்பட்டது | New Ticket Opened")
        .addFields(
          { name: "👤 User", value: `<@${member.id}>`, inline: true },
          { name: "📂 Type", value: `${cfg.emoji} ${cfg.label}`, inline: true },
          { name: "📌 Channel", value: `<#${ch.id}>`, inline: true },
        )
        .setFooter({ text: "⚡ YAMALOGAM | யாமாலோகம்" })
        .setTimestamp();
      interaction.guild.channels.cache.get(tc.logChannelId)?.send({ embeds: [logEmbed] }).catch(() => {});
    }
  } catch (e) {
    console.error("Ticket create error:", e);
    await interaction.editReply({ content: "❌ Ticket உருவாக்க முடியவில்லை! Bot permissions சரிபார்க்கவும்!" });
  }
}
...
async function handleCloseTicketButton(interaction) {
  ...
  const tc = getTC(interaction.guild.id);
  const userId = [...tc.openTickets.entries()].find(([, v]) => v.channelId === ch.id)?.[0];
  const ticketType = userId ? tc.openTickets.get(userId)?.type : null;
  const cfg = ticketType ? TICKET_TYPES[ticketType] : null;
  if (userId) tc.openTickets.delete(userId);

  const closeEmbed = new EmbedBuilder()
    .setColor(0xff4444)
    .setTitle("🔒 Ticket மூடப்படுகிறது | Ticket Closing")
    .setDescription("5 வினாடிகளில் இந்த ticket மூடப்படும்!\nThis ticket will be deleted in 5 seconds!")
    .setFooter({ text: "⚡ YAMALOGAM | யாமாலோகம்" })
    .setTimestamp();
  ...
  if (tc.logChannelId) {
    const logEmbed = new EmbedBuilder()
      .setColor(0xff4444)
      .setTitle("📋 Ticket மூடப்பட்டது | Ticket Closed")
      .addFields(
        { name: "👤 Closed by", value: `<@${interaction.member.id}>`, inline: true },
        { name: "📂 Type", value: cfg ? `${cfg.emoji} ${cfg.label}` : "Unknown", inline: true },
      )
      .setFooter({ text: "⚡ YAMALOGAM | யாமாலோகம்" })
      .setTimestamp();
    interaction.guild.channels.cache.get(tc.logChannelId)?.send({ embeds: [logEmbed] }).catch(() => {});
...
async function handleTicket(message, args) {
  ...
  if (sub === "setup") {
    ...
    await message.reply("✅ YAMALOGAM Ticket panel உருவாக்கப்பட்டது! | Ticket panel created!");

  } else if (sub === "close") {
    if (!message.channel.name.includes("ticket")) { await message.reply("⚠️ இது ticket channel மட்டுமே பயன்படுத்தவும்! | Only inside a ticket channel!"); return; }
    const userId = [...tc.openTickets.entries()].find(([, v]) => v.channelId === message.channel.id)?.[0];
    if (userId) tc.openTickets.delete(userId);
    await message.reply("🔒 5 வினாடிகளில் மூடப்படும்! | Closing in 5 seconds!");
    setTimeout(() => message.channel.delete().catch(() => {}), 5000);

  } else if (sub === "add") {
    const t = message.mentions.members?.first(); if (!t) { await message.reply("Mention a user!"); return; }
    await message.channel.permissionOverwrites.edit(t.id, { ViewChannel: true, SendMessages: true });
    await message.reply(`✅ <@${t.id}> சேர்க்கப்பட்டார்! | Added!`);

  } else if (sub === "remove") {
    const t = message.mentions.members?.first(); if (!t) { await message.reply("Mention a user!"); return; }
    await message.channel.permissionOverwrites.delete(t.id);
    await message.reply(`✅ <@${t.id}> நீக்கப்பட்டார்! | Removed!`);

  } else if (sub === "gamesupport") {
    if (!requirePerm(message, PermissionFlagsBits.Administrator)) return;
    const cat = message.mentions.channels.first();
    if (!cat || cat.type !== ChannelType.GuildCategory) { await message.reply("Category channel-ஐ mention செய்யுங்கள்!"); return; }
    tc.gameSupportCategoryId = cat.id;
    await message.reply(`✅ 🎮 Game Support category: **${cat.name}**`);

  } else if (sub === "buyproduct") {
    if (!requirePerm(message, PermissionFlagsBits.Administrator)) return;
    const cat = message.mentions.channels.first();
    if (!cat || cat.type !== ChannelType.GuildCategory) { await message.reply("Category channel-ஐ mention செய்யுங்கள்!"); return; }
    tc.buyProductCategoryId = cat.id;
    await message.reply(`✅ 🛒 Buy Product category: **${cat.name}**`);

  } else if (sub === "supportrole") {
    if (!requirePerm(message, PermissionFlagsBits.Administrator)) return;
    const role = message.mentions.roles.first(); if (!role) { await message.reply("Role-ஐ mention செய்யுங்கள்!"); return; }
    tc.supportRoleId = role.id;
    await message.reply(`✅ Support role: **${role.name}**`);

  } else if (sub === "logchannel") {
    if (!requirePerm(message, PermissionFlagsBits.Administrator)) return;
    const ch = message.mentions.channels.first(); if (!ch) { await message.reply("Channel-ஐ mention செய்யுங்கள்!"); return; }
    tc.logChannelId = ch.id;
    await message.reply(`✅ Ticket logs → <#${ch.id}>`);

  } else {
    await message.reply(
      "**🎫 YAMALOGAM Ticket Commands:**\n" +
      "`+ticket setup` — Ticket panel அனுப்பு\n" +
      "`+ticket gamesupport #category` — 🎮 Game Support category set\n" +
      "`+ticket buyproduct #category` — 🛒 Buy Product category set\n" +
      "`+ticket supportrole @role` — Support role set\n" +
      "`+ticket logchannel #channel` — Log channel set\n" +
      "`+ticket close` — Ticket மூடு\n" +
      "`+ticket add/remove @user` — User சேர்/நீக்கு"
    );
  }
}
...
async function handleHelp(message) {
  await message.reply({ embeds: [new EmbedBuilder().setColor(0x5865f2).setTitle("📖 Bot Commands").setDescription("All commands (prefix: `+`)")
    .addFields(
      ...
      { name: "🎫 Tickets", value: "`+ticket setup` `+ticket gamesupport/buyproduct #cat` `+ticket supportrole/logchannel` `+ticket close/add/remove`" },
