...
client.on('messageCreate', async (message) => {
  ...
  if (message.mentions.has(client.user)) {
    ...
    try {
      ...
      const aiEmbed = new EmbedBuilder()
        ...
        .setDescription(reply.slice(0, 2000))
  ...
  if (command === 'help') {
    const helpEmbed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '🤖 AI', value: 'Just mention the bot and ask anything! (Powered by Gemini)', inline: false }
      )
  ...
  if (command === 'roast') {
    ...
    try {
      await message.channel.sendTyping();
      const result = await model.generateContent(`Give a funny, savage but harmless roast for a Discord user named ${target.user.username}. Keep it short, playful, and not offensive.`);
  ...
  if (command === 'about') {
    const aboutEmbed = new EmbedBuilder()
      ...
      .addFields(
        { name: '🧠 AI', value: 'Powered by Gemini (Google) - Free!', inline: true },
